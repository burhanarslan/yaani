...
export class DummyInterceptor implements HttpInterceptor {
  ...
  public intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    if (request.url.includes('/list')) {
      return this.logAndResponse(request, this.dummyMovies);
    }

    if (request.url.includes('/delete')) {
      const movieId = request.params.get('movieId')
      this.dummyMovies = this.dummyMovies.filter(item => item.id !== movieId)
      return this.logAndResponse(request, {});
    }
  ...
  private logAndResponse(request: HttpRequest<any>, data: any): Observable<HttpEvent<any>> {
    return of(new HttpResponse({...request, body: data}));
  }
